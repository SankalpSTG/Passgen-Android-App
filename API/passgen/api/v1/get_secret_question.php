<?php
	...
	if(isset($_POST["unique_id"]) && isset($_POST["master_password"])){
		...
		if(!$message["error"]){
			$stmt = $conn->prepare("SELECT secret_question FROM pg_users WHERE unique_id = ?");
			$stmt->bind_param("s", $unique_id);
			if($stmt->execute()){
				$result = $stmt->get_result();
				$row = mysqli_fetch_assoc($result);
				$message = array("error" => true, "error_code" => 100, "message" => "successful", "data" => $row["secret_question"]);
